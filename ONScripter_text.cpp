...
#if defined(INSANI)
float ONScripter::drawGlyph(SDL_Surface *dst_surface, FontInfo *info,
                            SDL_Color &color, char *text, int xy[2],
                            AnimationInfo *cache_info, SDL_Rect *clip,
                            SDL_Rect &dst_rect)
#else
int ONScripter::drawGlyph(SDL_Surface *dst_surface, FontInfo *info,
                          SDL_Color &color, char *text, int xy[2],
                          AnimationInfo *cache_info, SDL_Rect *clip,
                          SDL_Rect &dst_rect)
#endif
{
...
#if defined(INSANI)
  int font_index = 0;
  if (!info->style_bold && !info->style_italics)
    font_index = 0;
  else if (info->style_bold && !info->style_italics)
    font_index = 2;
  else if (!info->style_bold && info->style_italics)
    font_index = 4;
  else if (info->style_bold && info->style_italics)
    ...
    font_index = 0;
  TTF_GlyphMetrics((TTF_Font *)info->ttf_font[0], unicode, &normal_minx,
                   &normal_maxx, &normal_miny, &normal_maxy, &normal_advanced);
  TTF_GlyphMetrics((TTF_Font *)info->ttf_font[font_index], unicode, &minx,
                   &maxx, &miny, &maxy, &advanced);
  // printf("min %d %d %d %d %d %d\n", minx, maxx, miny, maxy,
  // advanced,TTF_FontAscent((TTF_Font*)info->ttf_font[font_index])  );
#else
  TTF_GlyphMetrics((TTF_Font *)info->ttf_font[0], unicode, &minx, &maxx, &miny,
                   &maxy, &advanced);
  // printf("min %d %d %d %d %d %d\n", minx, maxx, miny, maxy,
  ...
  SDL_Surface *tmp_surface = TTF_RenderGlyph_Shaded(
      (TTF_Font *)info->ttf_font[font_index], unicode, fcol, bcol);
#else
  SDL_Surface *tmp_surface = TTF_RenderGlyph_Shaded(
  ...
  if (info->is_shadow && render_font_outline) {
    ...
    tmp_surface_s = TTF_RenderGlyph_Shaded(
        (TTF_Font *)info->ttf_font[font_index + 1], unicode, fcol, bcol);
#else
  ...
  // pick the TTF_Font in use
#if defined(INSANI)
  TTF_Font *font = (TTF_Font *)info->ttf_font[font_index];
#else
  TTF_Font *font = (TTF_Font *)info->ttf_font[0];
#endif

  // metrics
  int ascent = TTF_FontAscent(font);
  int line_h = TTF_FontHeight(font); // ascent + absolute(descent)
  int glyph_h = maxy - miny;         // actual glyph box height

  // compute vertical offset so that the glyph box is vertically centered
  // within the full line height, and then shift by its min-y
  int vert_off = (line_h - glyph_h) / 2 - miny;

  // final vertical position
  dst_rect.y = xy[1] + vert_off;
  ...
  if (info->is_shadow && tmp_surface_s) {
    ...
    if (cache_info)
      cache_info->blendText(tmp_surface_s, dst_rect_s.x, dst_rect_s.y, scolor,
                            clip, rotate_flag);
  ...
  if (tmp_surface) {
    ...
    if (cache_info)
      cache_info->blendText(tmp_surface, dst_rect.x, dst_rect.y, color, clip,
                            rotate_flag);
...
#if defined(INSANI)
  if (!english_mode ||
      (info->style_bold && !info->style_italics && faux_bold) ||
      (!info->style_bold && info->style_italics && faux_italics) ||
      (info->style_bold && info->style_italics && faux_bolditalics)) {
    // do not use harfbuzz metrics if we are in a faux style or we are not in
    // English mode
    return (float)advanced;
  } else {
    // for normal, true bold, true italics, and true bold italics, whilst in
    // UTF8 mode, use harfbuzz
    hb_buffer_t *hb_buf;
    ...
    if (!info->style_bold && !info->style_italics)
      ...
      blob = hb_blob_create_from_file(font_bold_file);
    else if (!info->style_bold && info->style_italics)
      blob = hb_blob_create_from_file(font_italics_file);
    else if (info->style_bold && info->style_italics)
      blob = hb_blob_create_from_file(font_bolditalics_file);
    ...
    if (num_chars == 2 && glyph_count == 1) {
      ...
      // glyph_pos[0].x_advance); printf("drawGlyph :: in ligature :: ptem: %d
      // :: upem :: %d\n", ptem, upem);
...
#if defined(INSANI)
    if (fi->openFont(font_file, font_bold_file, font_italics_file,
                     font_bolditalics_file, faux_bold, faux_italics,
                     faux_bolditalics, screen_ratio1, screen_ratio2) == NULL) {
#endif
...
void ONScripter::drawChar(char *text, FontInfo *info, bool flush_flag,
  ...
  for (int i = 0; i < 2; i++) {
    ...
    SDL_Rect dst_rect;
    float adv =
        drawGlyph(surface, info, color, text, xy, cache_info, clip, dst_rect);
#if not defined(INSANI)
    // this arguably doesn't even work well in monospaced English mode
    if (n == 1)
      adv -= 0.5; // 0.5 is for adjusting the increse by FT_CEIL
#endif
    ...
    if (n >= 2) {
      if (script_h.enc.getEncoding() == Encoding::CODE_UTF8) {
        ...
        info->advanceCharInHankaku(2);
#if defined(INSANI)
      // unless you do this, your non-1-byte characters will not have the
      // appropriate shadow under them until you go beyond a clickwait
      if (info->is_shadow) {
        if (render_font_outline)
          info->addShadeArea(dst_rect, -1, -1, 2, 2);
        else
...
#if defined(INSANI)
      if (out_text[0] == '\n') {
        if (f_info.is_newline_accepted == false)
          f_info.is_newline_accepted = true;
        f_info.newLine();
        continue;
      }
#endif
...
#if defined(INSANI)
                 (current_page->text[i + 1] == 'b' ||
                  current_page->text[i + 1] == 'i' ||
                  current_page->text[i + 1] == 'u') &&
                 (current_page->text[i + 2] == 'b' ||
                  current_page->text[i + 2] == 'i' ||
                  current_page->text[i + 2] == 'u' ||
                  current_page->text[i + 2] == '~') &&
#endif
                 out_text[0] == '~') {
        while (1) {
          ...
          if (ch == '~') {
#if not defined(INSANI)
            // This is actually a bug, and advances the buffer one character too
            // far, deleting the next true letter after the ~
            i++;
#endif
            break;
          }
...
bool ONScripter::doClickEnd() {
  ...
  if (automode_flag) {
    event_mode =
        WAIT_TEXT_MODE | WAIT_INPUT_MODE | WAIT_VOICE_MODE | WAIT_TIMER_MODE;
    if (automode_time < 0)
...
bool ONScripter::clickWait(char *out_text) {
  ...
  string_buffer_offset +=
      script_h.checkClickstr(script_h.getStringBuffer() + string_buffer_offset);

#if defined(INSANI)
  // this block is necessary because as it stands, onscripter will not properly
  // advance the string buffer offset past the @ symbol when in 1 byte mode,
  // causing an infinite stall.
  if (english_mode || legacy_english_mode) {
  ...
  if ((skip_mode & (SKIP_NORMAL | SKIP_TO_EOP) || ctrl_pressed_status) &&
    ...
    if (out_text) {
      drawChar(out_text, &sentence_font, false, true, accumulation_surface,
               &text_info);
    } else { // called on '@'
      flush(refreshMode());
    }
...
#if defined(INSANI)
  // reset all font styles with each new line; should not strictly be necessary,
  // as we do this in processText() at the end of the line; this is just to be
  // safe and for certain savefile load conditions
  sentence_font.setStyle(0, 0, 0, 0);
#endif
 ...
 * --
 * A simple function to grab the number of glyphs in a given UTF8-encoded
 * string. Works just like standard strlen.  Necessary for the initial version
 * of the insani legacy linewrap algorithm with UTF8-encoded 0.utf.
 */
 ...
 */
float ONScripter::strpxlen(const char *buf, FontInfo *fi, bool *bold_flag,
                           bool *italics_flag) {
  openFont(fi);
  ...
  while (buf[0] != '\0') {
    ...
    for (int y = 0; y < o; y++)
      two_chars[n + y] = buf[n + y];

    int minx, maxx, miny, maxy, advanced_int;
    TTF_GlyphMetrics((TTF_Font *)fi->ttf_font[font_index], unicode, &minx,
                     &maxx, &miny, &maxy, &advanced_int);

    advanced = (float)advanced_int;

    if (!english_mode || (fi->style_bold && !fi->style_italics && faux_bold) ||
        (!fi->style_bold && fi->style_italics && faux_italics) ||
        (fi->style_bold && fi->style_italics && faux_bolditalics)) {
      // do not use harfbuzz metrics if we are in a faux style or we are not in
      // English mode
    } else {
      // for normal, true bold, true italics, and true bold italics, whilst in
      ...
      hb_buffer_t *hb_buf;
      hb_buf = hb_buffer_create();
      hb_buffer_add_utf8(hb_buf, two_chars, strlen(two_chars), 0,
                         strlen(two_chars));
      hb_buffer_guess_segment_properties(hb_buf);

      hb_blob_t *blob = NULL;

      if (!fi->style_bold && !fi->style_italics)
        blob = hb_blob_create_from_file(font_file);
      else if (fi->style_bold && !fi->style_italics)
        blob = hb_blob_create_from_file(font_bold_file);
      else if (!fi->style_bold && fi->style_italics)
        blob = hb_blob_create_from_file(font_italics_file);
      else if (fi->style_bold && fi->style_italics)
 ...
 * commands.  A critical part of the insani linewrap algorithm for all non-CJK
 * modes.
 */
float ONScripter::getPixelLength(const char *buf, FontInfo *fi, bool *bold_flag,
                                 bool *italics_flag) {
  // openFont(fi);
  ...
  while (buf[0] != '\0') {
    ...
    if (buf[0] == '!') {
      ...
      // !sd
      else if (buf[1] == 's' && buf[2] == 'd')
        orig_length -= strpxlen("!sd", fi, bold_flag, italics_flag);
      // !s
      else if (buf[1] == 's') {
        tmp[0] = '!';
        tmp[1] = 's';
    ...
    // ~i~, ~b~, ~ib~, ~bi~
      ...
      if (buf[1] == 'b' && buf[2] == '~') {
        ...
        if (*italics_flag == true)
          ...
          *italics_flag = true;
      } else if ((buf[1] == 'b' && buf[2] == 'i' && buf[3] == 'u' &&
                  buf[4] == '~') ||
                 (buf[1] == 'b' && buf[2] == 'u' && buf[3] == 'i' &&
                  buf[4] == '~') ||
                 (buf[1] == 'i' && buf[2] == 'b' && buf[3] == 'u' &&
                  buf[4] == '~') ||
                 (buf[1] == 'i' && buf[2] == 'u' && buf[3] == 'b' &&
                  buf[4] == '~') ||
                 (buf[1] == 'u' && buf[2] == 'b' && buf[3] == 'i' &&
                  buf[4] == '~') ||
                 (buf[1] == 'u' && buf[2] == 'i' && buf[3] == 'b' &&
                  buf[4] == '~')) {
        char biu[6];
        biu[0] = '~';
        biu[1] = buf[1];
...
void ONScripter::getNextChar(const char *buf, int offset, char *out_chars) {
  ...
  out_chars[0] = out_chars[1] = out_chars[2] = out_chars[3] = out_chars[4] =
      out_chars[5] = out_chars[6] = '\0';
  ...
  while (!printable) {
    ...
    else if (buf[i] == '~' && buf[i + 1] == 'b' && buf[i + 2] == '~')
      i += 3;
    else if (buf[i] == '~' && buf[i + 1] == 'i' && buf[i + 2] == '~')
      i += 3;
    else if (buf[i] == '~' && buf[i + 1] == 'u' && buf[i + 2] == '~')
      i += 3;
    else if (buf[i] == '~' && buf[i + 1] == 'b' && buf[i + 2] == 'i' &&
             buf[i + 3] == '~')
      i += 4;
    else if (buf[i] == '~' && buf[i + 1] == 'b' && buf[i + 2] == 'u' &&
             buf[i + 3] == '~')
      i += 4;
    else if (buf[i] == '~' && buf[i + 1] == 'i' && buf[i + 2] == 'b' &&
             buf[i + 3] == '~')
      i += 4;
    else if (buf[i] == '~' && buf[i + 1] == 'i' && buf[i + 2] == 'u' &&
             buf[i + 3] == '~')
      i += 4;
    else if (buf[i] == '~' && buf[i + 1] == 'u' && buf[i + 2] == 'b' &&
             buf[i + 3] == '~')
      i += 4;
    else if (buf[i] == '~' && buf[i + 1] == 'u' && buf[i + 2] == 'i' &&
             buf[i + 3] == '~')
      i += 4;
    else if (buf[i] == '~' && buf[i + 1] == 'b' && buf[i + 2] == 'i' &&
             buf[i + 3] == 'u' && buf[i + 4] == '~')
...
int ONScripter::textCommand() {
  ...
  if (pretextgosub_label && !last_nest_info->pretextgosub_flag &&
  ...
  if (english_mode) {
    ...
    float line_length =
        (float)sentence_font.num_xy[0] * (float)sentence_font.pitch_xy[0];
    ...
    if (prev_skip_newline_mode &&
        original_text[strlen(original_text) - 1] == '/')
      skip_newline_mode = true;

    if (!prev_skip_newline_mode)
      skip_newline_offset = 0.0;
    else if (prev_skip_newline_mode)
      current_line_length = skip_newline_offset;

    // in ScriptHandler.cpp: #define STRING_BUFFER_LENGTH 4096 -- this is the
    // max string buffer length ONScripter supports.
    char *temp_text = (char *)malloc(4096 * sizeof(char));

    float original_text_length = getPixelLength(
        original_text, &sentence_font, &wrap_style_bold, &wrap_style_italics);
    ...
    if (&current_word[0] != &temp_text[0]) {
      ...
      for (int i = 0; original_text[i] == ' '; i++) {
        temp_text[i] = ' ';
        space_counter += strpxlen(" ", &sentence_font, &wrap_style_bold,
                                  &wrap_style_italics);
      }
      if (current_line_length +
              getPixelLength(current_word, &sentence_font, &wrap_style_bold,
                             &wrap_style_italics) +
              space_counter <=
          line_length)
        current_word = temp_text;
    }

    // now parse special cases
    if (getPixelLength(current_word, &sentence_font, &wrap_style_bold,
                       &wrap_style_italics) >= line_length) {
      // ... then there's no point in word-wrapping at all, as the word that the
      // writer/translator put in is longer than the limit anyway, so do nothing
    } else if (original_text_length + current_line_length <= line_length) {
      // ... the entire text fits on one display line anyway, so just calculate
      // the new skip_newline_offset
      if (skip_newline_mode) {
        ...
        while (char_is_token) {
          if (original_text[char_is_token_counter] == '\\' ||
              ...
              original_text[char_is_token_counter] == '/') {
            if (original_text[char_is_token_counter] == '\\')
              original_text_length -= strpxlen(
                  "\\", &sentence_font, &wrap_style_bold, &wrap_style_italics);
            else if (original_text[char_is_token_counter] == '@')
              original_text_length -= strpxlen(
                  "@", &sentence_font, &wrap_style_bold, &wrap_style_italics);
            else if (original_text[char_is_token_counter] == '/')
              original_text_length -= strpxlen(
      ...
      if (current_line_length +
              getPixelLength(current_word, &sentence_font, &wrap_style_bold,
                             &wrap_style_italics) +
              strpxlen(" ", &sentence_font, &wrap_style_bold,
                       &wrap_style_italics) <=
          line_length) {
        ...
        // we capture this in a variable because we need to reset this to 0
        // whenever we line break
        current_line_length +=
            getPixelLength(original_text, &sentence_font, &wrap_style_bold,
                           &wrap_style_italics);
        if (skip_newline_mode)
        ...
        // the only time the above won't work is if we had a / before and need
        // to word wrap on the first word
        strcpy(original_text, "");
        strcat(original_text, "\n");

        // now we've begun the new line, and we set the current line length to
        // the current word
      ...
      while (current_word != NULL) {
        ...
        if (next_word != NULL &&
            ((strlen(next_word) == 4 && (unsigned char)next_word[0] == 0xe2 &&
              (unsigned char)next_word[1] == 0x80 &&
              (unsigned char)next_word[2] == 0x94) ||
             (strlen(next_word) == 4 && (unsigned char)next_word[0] == 0xe2 &&
              (unsigned char)next_word[1] == 0x80 &&
              (unsigned char)next_word[2] == 0x95) ||
             (strlen(current_word) == 3 &&
              (unsigned char)current_word[0] == 0xc2 &&
              (unsigned char)current_word[1] == 0xab) ||
             (strlen(current_word) == 4 &&
              (unsigned char)current_word[0] == 0xe2 &&
              (unsigned char)current_word[1] == 0x80 &&
              (unsigned char)current_word[2] == 0xb9) ||
             (strlen(next_word) == 3 && (unsigned char)next_word[0] == 0xc2 &&
              (unsigned char)next_word[1] == 0xbb) ||
             (strlen(next_word) == 4 && (unsigned char)next_word[0] == 0xe2 &&
              (unsigned char)next_word[1] == 0x80 &&
              (unsigned char)next_word[2] == 0xba) ||
             strcmp(next_word, "-") == 0 || strcmp(next_word, "--") == 0 ||
             (next_word[0] == '.' && next_word[1] == '.' &&
              next_word[2] == '.'))) {
          // attempt to replace the null character that terminates current_word
          // with a space; should have the effect of concatenating the two
          // strings together
          ...
          // replace the null character that terminates current_word with a
          // space; this will not, unlike above, concatenate current_word and
          // next_word together because there is another space
          null_index = null_index - 2;
          current_word[null_index] = ' ';
        ...
        if (current_line_length +
                ...
                strpxlen(" ", &sentence_font, &wrap_style_bold,
                         &wrap_style_italics) <=
            line_length) {
          // ... the length of the wrapped text and the current word are still
          // less than the max line length, so ...
        ...
        // printf("%s %d %d\n", current_word, getPixelLength(current_word,
        // &sentence_font, &wrap_style_bold, &wrap_style_italics),
        // current_line_length);
...
bool ONScripter::checkLigatureLineBreak(const char *buf, FontInfo *fi) {
  if (script_h.current_language != 0)
    return false;
  ...
  while (buf[0] != ' ' && buf[0] != 0x0a && buf[0] != 0 &&
    ...
    int minx, maxx, miny, maxy, advanced;
    TTF_GlyphMetrics((TTF_Font *)fi->ttf_font[0], unicode, &minx, &maxx, &miny,
                     &maxy, &advanced);
...
bool ONScripter::processText() {
...
#if defined(INSANI)
  // deal with the case of the line that starts with ~ (as we insert the ` back
  // when this occurs)
  if (ch == '`') {
    string_buffer_offset++;
  ...
  if (n >= 2) {
...
#if defined(INSANI)
      drawChar(out_text_2, &sentence_font, false, true, accumulation_surface,
               &text_info);
#else
      drawChar(out_text, &sentence_font, false, true, accumulation_surface,
               &text_info);
    ...
    if (sentence_font.is_newline_accepted == false)
    ...
    if (script_h.getStringBuffer()[string_buffer_offset + 1] == 's' ||
      ...
      out_text[0] = script_h.getStringBuffer()[string_buffer_offset];
      getNextChar(script_h.getStringBuffer(), string_buffer_offset, out_text_2);
      drawChar(out_text_2, &sentence_font, true, true, accumulation_surface,
               &text_info);
      string_buffer_offset++;
      num_chars_in_sentence++;
      return true;
    ...
    return true;
  } else if (ch == '(' && (!english_mode || !(script_h.getEndStatus() &
                                              ScriptHandler::END_1BYTE_CHAR))) {
    current_page->add('(');
...
#if defined(INSANI)
             (script_h.getStringBuffer()[string_buffer_offset + 1] == 'b' ||
              script_h.getStringBuffer()[string_buffer_offset + 1] == 'i' ||
              script_h.getStringBuffer()[string_buffer_offset + 1] == 'u') &&
             (script_h.getStringBuffer()[string_buffer_offset + 2] == 'b' ||
              script_h.getStringBuffer()[string_buffer_offset + 2] == 'i' ||
              script_h.getStringBuffer()[string_buffer_offset + 2] == 'u' ||
    ...
    while (1) {
      ...
      if (ch == 'i') {
        ...
        // printf("style_bold: %d :: style_italics: %d :: style_underline:
        // %d\n", sentence_font.style_bold, sentence_font.style_italics,
        // sentence_font.style_underline);
      } else if (ch == 'b') {
        openFont(&sentence_font);
    ...
    if (matched_len > 0) {
      ...
      if (sentence_font.getRemainingLine() <= clickstr_line)
        ...
        return clickWait(out_text);
               ...
               script_h.getEndStatus() & ScriptHandler::END_1BYTE_CHAR) {
#if defined(INSANI)
      // reset all font styles at the end of each line -- this is no longer
      // necessary as of release "D3"
      // sentence_font.setStyle(0, 0, 0, 0);
#endif
      if (script_h.getStringBuffer()[string_buffer_offset + 2] &&
          script_h.checkClickstr(
              &script_h.getStringBuffer()[string_buffer_offset + 2]) > 0) {
        clickstr_state = CLICK_NONE;
      } else if (script_h.getStringBuffer()[string_buffer_offset + 1] == '@') {
        return clickWait(out_text);
      } else if (script_h.getStringBuffer()[string_buffer_offset + 1] == '\\') {
    ...
    if (script_h.getStringBuffer()[string_buffer_offset + 1] &&
        ...
        script_h.enc.getEncoding() == Encoding::CODE_CP932) {
      out_text[1] = script_h.getStringBuffer()[string_buffer_offset + 1];
      drawChar(out_text, &sentence_font, flush_flag, true, accumulation_surface,
               &text_info);
      num_chars_in_sentence++;
    } else if (script_h.getEndStatus() & ScriptHandler::END_1BYTE_CHAR ||
               script_h.enc.getEncoding() == Encoding::CODE_UTF8) {
      if (checkLigatureLineBreak(script_h.getStringBuffer() +
                                     string_buffer_offset,
                                 &sentence_font))
        sentence_font.newLine();
#if defined(INSANI)
      getNextChar(script_h.getStringBuffer(), string_buffer_offset, out_text_2);
      drawChar(out_text_2, &sentence_font, flush_flag, true,
               accumulation_surface, &text_info);
#else
      drawChar(out_text, &sentence_font, flush_flag, true, accumulation_surface,
               &text_info);
