...
int ONScripter::splitCommand() {
  ...
  while (script_h.getEndStatus() & ScriptHandler::END_COMMA) {
    ...
    if (script_h.current_variable.type & ScriptHandler::VAR_INT ||
      ...
      setStr(&script_h.getVariableData(script_h.current_variable.var_no).str,
             token);
    }
...
int ONScripter::selectCommand() {
  ...
  while (1) {
    if (script_h.getNext()[0] != 0x0a && comma_flag == true) {
      ...
      comma_flag = (script_h.getEndStatus() & ScriptHandler::END_COMMA);
      // printf("2 comma %d %c %x\n", comma_flag, script_h.getCurrent()[0],
      // script_h.getCurrent()[0]);
  ...
  if (select_mode != SELECT_CSEL_MODE) {
    ...
    while (last_select_link) {
      if (*last_select_link->text) {
        ButtonLink *button =
            getSelectableSentence(last_select_link->text, &sentence_font);
        root_button_link.insert(button);
        button->no = counter;
      }
      counter++;
...
int ONScripter::ofscopyCommand() {
  ...
  SDL_RenderReadPixels(renderer, &rect, tmp_surface->format->format,
                       tmp_surface->pixels, tmp_surface->pitch);
...
int ONScripter::movieCommand() {
  ...
  if (playMPEG(filename, click_flag, loop_flag, nosound_flag))
    endCommand();
  ...
  if (no >= 0) {
    ...
    if (ai->image_surface) {
      ...
      ai->orig_pos.x =
          ...
          ai->orig_pos.w / 2;
      ai->orig_pos.y = underline_value -
                       ai->image_surface->h * screen_ratio2 / screen_ratio1;
      ai->scalePosXY(screen_ratio1, screen_ratio2);
      dirty_rect.add(ai->pos);
    }
  }
...
int ONScripter::layermessageCommand() {
  ...
  if (&sprite_info[layer_info[no].sprite_num] == smpeg_info) {
    if (strncmp(buf, "open/", 5) == 0) {
      ...
      layer_smpeg_sample =
          SMPEG_new_rwops(SDL_RWFromMem(layer_smpeg_buffer, length), NULL, 0);
...
int ONScripter::getscreenshotCommand() {
  ...
  SDL_RenderReadPixels(renderer, &rect, screenshot_surface->format->format,
...
int ONScripter::getregCommand() {
  ...
  if (!found_flag)
    fprintf(stderr, "  The key is not found.\n");
  fclose(fp);
...
int ONScripter::getmouseposCommand() {
  script_h.readInt();
  script_h.setInt(&script_h.current_variable,
                  current_button_state.x * screen_ratio2 / screen_ratio1);

  script_h.readInt();
  script_h.setInt(&script_h.current_variable,
                  current_button_state.y * screen_ratio2 / screen_ratio1);
...
int ONScripter::drawsp3Command() {
  ...
  ai->blendOnSurface2(accumulation_surface, x, y, screen_rect, layer_alpha_buf,
                      alpha);
...
int ONScripter::bltCommand() {
  ...
  if (sw == dw && sw > 0 && sh == dh && sh > 0) {
    ...
    dst_rect.x = dst_rect.x * screen_device_width / screen_width +
                 (device_width - screen_device_width) / 2;
    dst_rect.y = dst_rect.y * screen_device_width / screen_width +
                 (device_height - screen_device_height) / 2;
    dst_rect.w = dst_rect.w * screen_device_width / screen_width;
    dst_rect.h = dst_rect.h * screen_device_width / screen_width;
    SDL_Texture *texture =
        SDL_CreateTextureFromSurface(renderer, btndef_info.image_surface);
    ...
    for (int i = dy; i != dy + dh; i += step_y) {
      ...
      for (int j = dx; j != dx + dw; j += step_x) {
        ...
        if (x < 0 || x >= btndef_info.image_surface->w || y < 0 ||
            y >= btndef_info.image_surface->h)
          *(dst_buf + j) = 0;
        else
      ...
      for (i = 0; i < 2; i++) {
        ...
        while (start[4 + i][j] >= '0' && start[4 + i][j] <= '9')
          ruby_struct.font_size_xy[i] =
              ruby_struct.font_size_xy[i] * 10 + start[4 + i][j++] - '0';
      }
    case 5:
      i = 0;
