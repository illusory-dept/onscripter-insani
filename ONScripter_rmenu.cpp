...
void ONScripter::leaveSystemCall(bool restore_flag) {
...
#if defined(INSANI)
  if (system_menu_mode == SYSTEM_LOOKBACK)
    sentence_font.setStyle(cached_ttf_style, cached_style_bold,
                           cached_style_italics, cached_style_underline);
#endif
  ...
  if (restore_flag) {
    ...
    if (event_mode & WAIT_BUTTON_MODE) {
      ...
      // SDL_WarpMouse is no longer available in SDL2; warp within the window.
...
void ONScripter::executeSystemMenu() {
  ...
  if (current_button_state.button == -1) {
    if (menuselectvoice_file_name[MENUSELECTVOICE_CANCEL])
      playSound(menuselectvoice_file_name[MENUSELECTVOICE_CANCEL], SOUND_CHUNK,
                false, MIX_WAVE_CHANNEL);
    leaveSystemCall();
    return;
  }
...
bool ONScripter::executeSystemLoad() {
  ...
  for (unsigned int i = 1; i <= num_save_file; i++) {
    ...
    if (save_file_info.valid) {
      ...
      sprintf(buffer, format_str, save_item_name, save_file_info.sjis_no,
              save_file_info.sjis_month, save_file_info.sjis_day,
              save_file_info.sjis_hour, save_file_info.sjis_minute);
      nofile_flag = false;
    } else {
...
bool ONScripter::executeSystemYesNo(int caller, int file_no) {
  ...
  if (caller == SYSTEM_SAVE) {
    ...
    sprintf(name, format_str, save_item_name, save_file_info.sjis_no);
  ...
  if (current_button_state.button == 1) { // yes is selected
    if (menuselectvoice_file_name[MENUSELECTVOICE_YES])
      playSound(menuselectvoice_file_name[MENUSELECTVOICE_YES], SOUND_CHUNK,
                false, MIX_WAVE_CHANNEL);
    return true;
  } else {
    if (menuselectvoice_file_name[MENUSELECTVOICE_NO])
      playSound(menuselectvoice_file_name[MENUSELECTVOICE_NO], SOUND_CHUNK,
                false, MIX_WAVE_CHANNEL);
    return false;
  }
}
  ...
  if ((current_page->previous->text_count > 0) && current_page != start_page) {
    ...
    if (lookback_sp[0] >= 0) {
      ...
      button->image_rect.x = sentence_font_info.pos.x +
                             sentence_font_info.pos.w - button->anim[0]->pos.w;
  ...
  if (current_page->next != cached_page) {
    ...
    if (lookback_sp[1] >= 0) {
      ...
      button->image_rect.x = sentence_font_info.pos.x +
                             sentence_font_info.pos.w - button->anim[0]->pos.w;
      button->image_rect.y = sentence_font_info.pos.y +
                             sentence_font_info.pos.h - button->anim[0]->pos.h;
      button->image_rect.w = button->anim[0]->pos.w;
      button->image_rect.h = button->anim[0]->pos.h;
      button->anim[0]->pos.x = button->anim[1]->pos.x = button->image_rect.x;
      button->anim[0]->pos.y = button->anim[1]->pos.y = button->image_rect.y;
...
void ONScripter::buildDialog(bool yesno_flag, const char *mes1,
  ...
  SDL_Surface *s =
      SDL_CreateRGBSurface(SDL_SWSURFACE, DIALOG_W, DIALOG_H, fmt->BitsPerPixel,
  ...
  if (screen_ratio2 != screen_ratio1) {
    s2 = SDL_CreateRGBSurface(
        ...
        DIALOG_H * screen_ratio1 / screen_ratio2, fmt->BitsPerPixel, fmt->Rmask,
  ...
  for (int i = 0; i < 2; i++) {
    SDL_Surface *bs = SDL_CreateRGBSurface(
        SDL_SWSURFACE, DIALOG_BUTTON_W * 2, DIALOG_BUTTON_H, fmt->BitsPerPixel,
        fmt->Rmask, fmt->Gmask, fmt->Bmask, fmt->Amask);
    ...
    if (screen_ratio2 != screen_ratio1) {
      bs2 = SDL_CreateRGBSurface(
          SDL_SWSURFACE, DIALOG_BUTTON_W * 2 * screen_ratio1 / screen_ratio2,
          DIALOG_BUTTON_H * screen_ratio1 / screen_ratio2, fmt->BitsPerPixel,
          fmt->Rmask, fmt->Gmask, fmt->Bmask, fmt->Amask);
    ...
    for (int j = 0; j < 2; j++) {
      ...
      dialog_font.top_xy[0] =
          rect.x + (rect.w - dialog_font.pitch_xy[0] * strlen(mes[i]) / 2) / 2;
    ...
    btn->anim[0]->pos.x =
        dialog_info.pos.x + (DIALOG_W - 3 - (DIALOG_BUTTON_W + 8) * (2 - i)) *
                                screen_ratio1 / screen_ratio2;
    btn->anim[0]->pos.y =
        dialog_info.pos.y +
        (DIALOG_H - 3 - (DIALOG_FOOTER + DIALOG_BUTTON_H) / 2) * screen_ratio1 /
            screen_ratio2;
