...
int fseek_ons(FILE *stream, long offset, int whence) {
  ...
  return fseek(stream, fd_start_offset[fd] + fd_length[fd] + offset,
               SEEK_SET); // SEEK_END
}
...
FILE *fopen_ons(const char *path, const char *mode) {
  ...
  jlongArray jla = (jlongArray)jniEnv->CallObjectMethod(JavaONScripter,
                                                        JavaGetFD, jca, mode2);
  jlong jl[3];
...
int mkdir_ons(const char *pathname, mode_t mode) {
  if (mkdir(pathname, mode) == 0 || errno != EACCES)
    return 0;
...
#endif
{
  ...
  NSArray *cpaths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory,
  ...
  NSArray *dpaths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,
                                                        NSUserDomainMask, YES);
  NSString *dpath =
  ...
  if (CFURLGetFileSystemRepresentation(resourcesURL, TRUE,
                                       (UInt8 *)resourcesPath, 255))
    ons.setArchivePath(resourcesPath);
  ...
  if ([fm fileExistsAtPath:savePath isDirectory:NULL])
