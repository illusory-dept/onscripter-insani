...
void ONScripter::flushEventSub(SDL_Event &event) {
  if (event.type == ONS_MUSIC_EVENT) {
    ...
    playMIDI(midi_play_loop_flag);
             ...
             ONS_CHUNK_EVENT) { // for processing btntim2 and automode correctly
    if (wave_sample[event.user.code]) {
      ...
      if (event.user.code == MIX_LOOPBGM_CHANNEL0 && loop_bgm_name[1] &&
        ...
        Mix_PlayChannel(MIX_LOOPBGM_CHANNEL1, wave_sample[MIX_LOOPBGM_CHANNEL1],
                        -1);
    }
  }
}
...
bool ONScripter::waitEvent(int count) {
  ...
  while (1) {
    waitEventSub(count);
    if (system_menu_mode == SYSTEM_NULL || script_h.isExternalScript())
      break;
  ...
  if (event->button == SDL_BUTTON_RIGHT && event->type == SDL_MOUSEBUTTONUP &&
    ...
    if (event->type == SDL_MOUSEBUTTONDOWN)
      current_button_state.down_flag = true;
           ...
           (bexec_flag || (event_mode & WAIT_TEXT_MODE) ||
...
void ONScripter::variableEditMode(SDL_KeyboardEvent *event) {
  ...
  case SDLK_KP_ENTER:
    ...
    case EDIT_VARIABLE_NUM_MODE:
      script_h.setNumVariable(variable_edit_index,
  ...
  if (variable_edit_mode == EDIT_SELECT_MODE) {
    ...
    sprintf(wm_edit_string, "%s%s%d", EDIT_MODE_PREFIX, "Variable Index?  %",
            variable_edit_sign * variable_edit_num);
    ...
    sprintf(wm_edit_string, "%sCurrent %s=%d  New value? %s%d",
            EDIT_MODE_PREFIX, var_name, p, (variable_edit_sign == 1) ? "" : "-",
            variable_edit_num);
  }
...
bool ONScripter::keyPressEvent(SDL_KeyboardEvent *event) {
  ...
  if ((event_mode & (WAIT_INPUT_MODE | WAIT_BUTTON_MODE)) &&
    ...
    if (!useescspc_flag && event->keysym.sym == SDLK_ESCAPE) {
      ...
      sprintf(current_button_state.str, "WHEELDOWN");
                ...
                event->keysym.sym == SDLK_k || event->keysym.sym == SDLK_p) &&
               event_mode & WAIT_BUTTON_MODE) {
      shiftCursorOnButton(1);
      return false;
    } else if (((!getcursor_flag && event->keysym.sym == SDLK_DOWN) ||
                event->keysym.sym == SDLK_j || event->keysym.sym == SDLK_n) &&
               event_mode & WAIT_BUTTON_MODE) {
      shiftCursorOnButton(-1);
      return false;
  ...
  if (event_mode & WAIT_INPUT_MODE &&
      (autoclick_time == 0 || (event_mode & WAIT_BUTTON_MODE))) {
    if (event->keysym.sym == SDLK_RETURN ||
        event->keysym.sym == SDLK_KP_ENTER || event->keysym.sym == SDLK_SPACE) {
      if (!(event_mode & WAIT_TEXT_MODE))
        skip_mode |= SKIP_TO_EOL;
      playClickVoice();
