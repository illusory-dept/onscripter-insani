...
void ONScripter::setupAnimationInfo(AnimationInfo *anim, FontInfo *info) {
  if (anim->trans_mode != AnimationInfo::TRANS_STRING && anim->file_name &&
      anim->surface_name && strcmp(anim->file_name, anim->surface_name) == 0 &&
  ...
  if (anim->trans_mode == AnimationInfo::TRANS_STRING) {
...
#if defined(INSANI)
      f_info.openFont(font_file, font_bold_file, font_italics_file,
                      font_bolditalics_file, faux_bold, faux_italics,
                      faux_bolditalics, screen_ratio1, screen_ratio2);
#else
    ...
    if (anim->is_tight_region) {
      drawString(anim->file_name, anim->color_list[anim->current_cell], &f_info,
                 false, NULL, &pos);
    } else {
      int xy_bak[2];
    ...
    for (int i = 0; i < anim->num_of_cells; i++) {
      f_info.clear();
      drawString(anim->file_name, anim->color_list[i], &f_info, false, NULL,
                 NULL, anim);
    ...
    int location;
    SDL_Surface *surface =
        loadImage(anim->file_name, anim->is_flipped, &has_alpha, &location,
                  &anim->default_alpha);
    ...
    if (surface &&
      ...
      if (script_h.screen_width == 640 && anim->is_2x) {
        ...
        anim->orig_pos.w /= 2;
        anim->orig_pos.h /= 2;
        if (anim->orig_pos.w == 0)
          anim->orig_pos.w = 1;
        if (anim->orig_pos.h == 0)
          anim->orig_pos.h = 1;
      }
      if (w == 0)
        w = 1;
