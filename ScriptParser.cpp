...
#define VERSION_STR2                                                           \
  "Copyright (C) 2001-2020 Studio O.G.A. All Rights Reserved."
...
#if defined(INSANI)
  // for insani purposes, use the save directory for envdata -- this is
  // especially important when building Ventura-compatible macOS app bundles
  use_save_dir = true;
#endif
...
#ifdef ANDROID
  __android_log_print(
      ANDROID_LOG_ERROR, "ONS", " *** Error at %s:%d [%s]; %s ***\n",
      current_label_info.name, current_line, str, reason ? reason : "");
#else
  fprintf(stderr, " *** Error at %s:%d [%s]; %s ***\n", current_label_info.name,
          current_line, str, reason ? reason : "");
#endif
  exit(-1);
}
...
int ScriptParser::readEffect(EffectLink *effect) {
  ...
  // printf("readEffect %d: %d %d %s\n", num, effect->effect, effect->duration,
...
void ScriptParser::setKinsoku(const char *start_chrs, const char *end_chrs,
  ...
  for (int i = 0; i < num_start; i++) {
    start_kinsoku[num_start_kinsoku + i].unicode =
        script_h.enc.getUTF16(kchr, code);
    kchr += script_h.enc.getBytes(*kchr, code);
  }
  ...
  for (int i = 0; i < num_end; i++) {
    end_kinsoku[num_end_kinsoku + i].unicode =
        script_h.enc.getUTF16(kchr, code);
    kchr += script_h.enc.getBytes(*kchr, code);
  }
...
bool ScriptParser::isEndKinsoku(const char *str) {
  ...
  for (int i = 0; i < num_end_kinsoku; i++)
    if (unicode == end_kinsoku[i].unicode)
      return true;
  return false;
}
