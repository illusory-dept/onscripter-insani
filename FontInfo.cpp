...
#if defined(INSANI)
  ttf_font[2] = ttf_font[3] = ttf_font[4] = ttf_font[5] = ttf_font[6] =
      ttf_font[7] = NULL;
...
#if defined(INSANI)
void *FontInfo::openFont(char *font_file, char *font_bold_file,
                         char *font_italics_file, char *font_bolditalics_file,
                         bool ons_faux_bold, bool ons_faux_italics,
                         bool ons_faux_bolditalics, int ratio1, int ratio2)
#else
...
#endif
{
  ...
  if (!fc->next) {
    ...
    fc->next->font[0] =
        TTF_OpenFontRW(fc->next->rw_ops, SDL_TRUE, font_size * ratio1 / ratio2);
...
#if defined(INSANI)
  if (style == TTF_STYLE_ITALIC && !style_italics)
    style_italics = true;
  else if (style == TTF_STYLE_ITALIC && style_italics)
    style_italics = false;
  if (style == TTF_STYLE_BOLD && !style_bold)
    style_bold = true;
  else if (style == TTF_STYLE_BOLD && style_bold)
    style_bold = false;
  if (style == TTF_STYLE_UNDERLINE && !style_underline)
    style_underline = true;
  else if (style == TTF_STYLE_UNDERLINE && style_underline)
    style_underline = false;
...
int FontInfo::y(bool use_ruby_offset) {
  ...
  if (!is_line_space_fixed && enc->getEncoding() == Encoding::CODE_UTF8 &&
      ttf_font[0])
    pitch_y +=
        TTF_FontLineSkip((const TTF_Font *)ttf_font[0]) - font_size_xy[1];
...
void FontInfo::setLineArea(const char *buf) {
  if (enc->getEncoding() == Encoding::CODE_UTF8) {
    ...
    while (buf[0]) {
      ...
      TTF_GlyphMetrics((TTF_Font *)ttf_font[0], unicode, &minx, &maxx, &miny,
                       &maxy, &advanced);
