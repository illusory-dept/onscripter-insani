...
readTokenTop:
  string_counter = 0;
  char ch = *buf;
  if (ch == ';' || (strncmp(buf, "langjp", 6) == 0 && current_language == 0) ||
  ...
  } else if (ch & 0x80 || (ch >= '0' && ch <= '9') || ch == '@' || ch == '\\' ||
             ch == '/' || ch == '%' || ch == '?' || ch == '$' || ch == '[' ||
             ch == '(' || ch == '<' ||
    ...
    while (1) {
      ...
      if (n >= 2) {
        ...
        addStringBuffer(ch);
        buf++;
        if (!wait_script && !ignore_clickstr_flag && checkClickstr(old_buf) > 0)
          wait_script = buf;
    ...
    ch = *++buf;
#if defined(INSANI)
    // deal with the case of the line that begins with ~; currently, it treats
    // such lines as commands; so we insert the ` right back and deal with that
    // ` in ONScripter::processText
    if (ch == '~')
      addStringBuffer('`');
#endif
    ...
    if (string_buffer[buf_last_char] == '/')
...
int ScriptHandler::checkClickstr(const char *buf, bool recursive_flag) {
  ...
  while (click_buf[0]) {
    ...
    if (double_byte_check) {
      ...
      if (u1 == u2) {
        if (!recursive_flag && checkClickstr(buf + n, true) > 0)
          return 0;
        return n;
      }
      click_buf += n;
...
struct ScriptHandler::LabelInfo
ScriptHandler::lookupLabelNext(const char *label) {
  int i = findLabel(label);
...
void ScriptHandler::errorAndExit(const char *str) {
  ...
  __android_log_print(ANDROID_LOG_ERROR, "ONS",
                      " **** Script error, %s [%s] ***", str, string_buffer);
#else
...
int ScriptHandler::readScript(char *path) {
  ...
  if (fp == NULL) {
#if defined(INSANI)
    fprintf(stderr, "can't open any of 0.txt, 00.txt, 0.utf, 00.utf, "
                    "nscript.dat, nscr_sec.dat, nscript.___, pscript.dat\n");
#else
    fprintf(stderr,
...
int ScriptHandler::readScriptSub(FILE *fp, char **buf, int encrypt_mode) {
  ...
  if (encrypt_mode == 3 && !key_table_flag)
    errorAndExit(
        "readScriptSub: the EXE file must be specified with --key-exe option.");
