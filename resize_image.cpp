  ...
  if (byte_per_pixel >= 3) {
    ...
    for (i = 0; i < src_height; i++) {
      ...
      for (s = 0; s < byte_per_pixel; s++) {
        tmp_acc[s] = 0;
        tmp_acc_num[s] = 0;
        for (j = 0; j < -interpolation_width / 2 + interpolation_width - 1;
             j++) {
          if (j >= src_width)
            break;
      ...
      for (j = 0; j < src_width; j++)
        calcWeightedSum(&tmp_buf, &src_buf, j, interpolation_width, src_width,
                        byte_per_pixel);
      tmp_buf += tmp_offset;
    }
  } else {
      ...
      if (palette_flag) { // assuming byte_per_pixel=1
        *dst_buf++ = tmp_buffer[k];
        ...
        for (s = 0; s < byte_per_pixel; s++, k++) {
          ...
          p += dx * dy *
               tmp_buffer[k + mx * byte_per_pixel + my * tmp_total_width];
