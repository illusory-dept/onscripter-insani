...
AnimationInfo &AnimationInfo::operator=(const AnimationInfo &anim) {
  if (this != &anim) {
    ...
    if (image_surface) {
      image_surface = allocSurface(anim.image_surface->w, anim.image_surface->h,
                                   texture_format);
...
#define ADDBLEND_PIXEL()                                                       \
  {                                                                            \
    ...
    Uint32 d1 = (*dst_buffer & 0xff00ff) +                                     \
                ((((*src_buffer & 0xff00ff) * sa) >> 8) & 0xff00ff);           \
    d1 |= ((d1 & 0xff000000) ? 0xff0000 : 0) |                                 \
          ((d1 & 0x0000ff00) ? 0x0000ff : 0);                                  \
    Uint32 d2 =                                                                \
        (*dst_buffer & 0x00ff00) + (((*src_buffer & 0x00ff00) * sa) >> 8);     \
...
#define ADD2BLEND_PIXEL()                                                      \
  {                                                                            \
    ...
    Uint32 d1 = ((((*dst_buffer & 0xff00ff) * sa) >> 8) & 0xff00ff) +          \
                (*src_buffer & 0xff00ff);                                      \
    d1 |= ((d1 & 0xff000000) ? 0xff0000 : 0) |                                 \
          ((d1 & 0x0000ff00) ? 0x0000ff : 0);                                  \
    Uint32 d2 = ((((*dst_buffer & 0x00ff00) * sa) >> 8) & 0x00ff00) +          \
                (*src_buffer & 0x00ff00);                                      \
    d2 |= ((d2 & 0x00ff0000) ? 0x00ff00 : 0);                                  \
    *dst_buffer = (d1 & 0xff00ff) | (d2 & 0x00ff00) | 0xff000000;              \
  }
...
void AnimationInfo::blendOnSurface(SDL_Surface *dst_surface, int dst_x,
                                   int dst_y, SDL_Rect &clip,
                                   unsigned char *layer_alpha_buf, int alpha) {
  ...
  ONSBuf *src_buffer = (ONSBuf *)image_surface->pixels + pitch * src_rect.y +
                       image_surface->w * current_cell / num_of_cells +
                       src_rect.x;
  ONSBuf *dst_buffer =
      (ONSBuf *)dst_surface->pixels + dst_surface->w * dst_rect.y + dst_rect.x;
  unsigned char *lalphap =
      layer_alpha_buf + dst_surface->w * dst_rect.y + dst_rect.x;
#if defined(BPP16)
  ...
  for (int i = 0; i < dst_rect.h; i++) {
    for (int j = dst_rect.w; j != 0;
         j--, src_buffer++, dst_buffer++, lalphap++) {
  ...
  for (y = min_xy[1]; y <= max_xy[1]; y++) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (corner_xy[i][1] == corner_xy[i2][1])
        continue;
      x = (corner_xy[i2][0] - corner_xy[i][0]) * (y - corner_xy[i][1]) /
    ...
    for (x = raster_min - dst_x; x <= raster_max - dst_x;
         x++, dst_buffer++, lalphap++) {
      int x2 = ((inv_mat[0][0] * x >> 9) + x_offset2) >> 1;
      int y2 = ((inv_mat[1][0] * x >> 9) + y_offset2) >> 1;

      if (x2 < src_rect[0][0] || x2 > src_rect[1][0] || y2 < src_rect[0][1] ||
          y2 > src_rect[1][1])
        continue;

      ONSBuf *src_buffer = (ONSBuf *)image_surface->pixels + pitch * y2 + x2 +
                           pos.w * current_cell;
#if defined(BPP16)
      unsigned char *alphap =
          alpha_buf + image_surface->w * y2 + x2 + pos.w * current_cell;
#endif
...
#if defined(BPP16)
  for (i = 0; i < _src_rect.h; i++)
    memset(alpha_buf + image_surface->w * (_dst_rect.y + i) + _dst_rect.x, 0xff,
           _src_rect.w);
#endif
...
void AnimationInfo::fill(Uint8 r, Uint8 g, Uint8 b, Uint8 a) {
  ...
  SDL_PixelFormat *fmt = image_surface->format;
  Uint32 rgb =
      (((r >> fmt->Rloss) << fmt->Rshift) | ((g >> fmt->Gloss) << fmt->Gshift) |
...
SDL_Surface *AnimationInfo::setupImageAlpha(SDL_Surface *surface,
  ...
  if (trans_mode == TRANS_TOPLEFT) {
    ...
    ref_color = direct_color[0] << fmt->Rshift |
                direct_color[1] << fmt->Gshift | direct_color[2] << fmt->Bshift;
  }
  ref_color &= 0xffffff;
  ...
  if (trans_mode == TRANS_ALPHA && !has_alpha) {
    ...
    SDL_Surface *surface2 =
        SDL_CreateRGBSurface(SDL_SWSURFACE, w3, h, fmt->BitsPerPixel,
...
void AnimationInfo::setImage(SDL_Surface *surface, Uint32 texture_format) {
  ...
  for (int i = 0; i < surface->h; i++) {
    ONSBuf *dst_buffer = (ONSBuf *)((unsigned char *)image_surface->pixels +
                                    image_surface->pitch * i);
    Uint32 *buffer =
        (Uint32 *)((unsigned char *)surface->pixels + surface->pitch * i);
    for (int j = 0; j < surface->w; j++, buffer++) {
      // ARGB8888 -> RGB565 + alpha
      *dst_buffer++ =
          ((((*buffer) & 0xf80000) >> 8) | (((*buffer) & 0x00fc00) >> 5) |
           (((*buffer) & 0x0000f8) >> 3));
      *alphap++ = ((*buffer) >> 24);
    }
  }
...
#define SUBTRACT_PIXEL()                                                       \
  {                                                                            \
    ...
    Uint32 da = (sa ^ 0xff);                                                   \
    sa++;                                                                      \
    Uint32 d1 = (*src1_buffer & 0xff00ff) -                                    \
                ((((*src2_buffer & 0xff00ff) * sa) >> 8) & 0xff00ff);          \
    d1 &= ((d1 & 0xff000000) ? 0 : 0xff0000) |                                 \
          ((d1 & 0x0000ff00) ? 0 : 0x0000ff);                                  \
    Uint32 d2 =                                                                \
        (*src1_buffer & 0x00ff00) - (((*src2_buffer & 0x00ff00) * sa) >> 8);   \
    d2 &= (d2 & 0x00ff0000) ? 0 : 0x00ff00;                                    \
    *dst_buffer = d1 | d2 | da << 24;                                          \
  }
#endif

// effect_src_info = (surface(accumulation_surface) - layer_alpha *
// layer_info)/(0xff ^ layer_alpha) effect_src_info.alpha = 0xff ^ layer_alpha
void AnimationInfo::subtract(SDL_Surface *surface, AnimationInfo *layer_info,
                             unsigned char *layer_alpha_buf) {
  if (image_surface == NULL || layer_info->image_surface == NULL)
    return;
  ...
  for (int i = 0; i < surface->h; i++) {
    for (int j = surface->w; j != 0;
         j--, dst_buffer++, src1_buffer++, src2_buffer++, lalphap++) {
      SUBTRACT_PIXEL();
    }
  }
