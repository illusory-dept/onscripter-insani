...
void ONScripter::generateEffectSrc(bool update) {
  if (smpeg_info && smpeg_info->visible) {
    if (update) {
      ...
      smpeg_info->blendOnSurface(effect_src_surface, 0, 0, clip,
                                 layer_alpha_buf);
      effect_src_info.blendOnSurface(effect_src_surface, 0, 0, clip,
                                     layer_alpha_buf);
    } else {
      effect_src_info.subtract(accumulation_surface, smpeg_info,
                               layer_alpha_buf);
...
bool ONScripter::doEffect(EffectLink *effect, bool clear_dirty_region) {
  ...
  // printf("effect_counter %d timer between %d
  ...
  case 6: // Left curtain
    ...
    for (i = 0; i <= screen_width / EFFECT_STRIPE_CURTAIN_WIDTH; i++) {
      width2 = width - EFFECT_STRIPE_CURTAIN_WIDTH *
                           EFFECT_STRIPE_CURTAIN_WIDTH * i / screen_width;
      if (width2 >= 0) {
  ...
  case 8: // Top curtain
    ...
    for (i = 0; i <= screen_height / EFFECT_STRIPE_CURTAIN_WIDTH; i++) {
      height2 = height - EFFECT_STRIPE_CURTAIN_WIDTH *
                             EFFECT_STRIPE_CURTAIN_WIDTH * i / screen_height;
      if (height2 >= 0) {
        src_rect.x = 0;
  ...
  case (MAX_EFFECT_NUM + 0): // quakey
    ...
    amp = (Sint16)(sin(M_PI * 2.0 * effect->no * effect_counter /
                       effect_duration) *
    ...
    SDL_FillRect(accumulation_surface, &quake_rect,
