  ...
  if (num_of_ns2_archives == 0 && archive_type & ARCHIVE_TYPE_NSA) {
    for (int k = 0; k < 2; k++) {
      for (int i = -1; i < MAX_EXTRA_ARCHIVE; i++) {
        ...
        if (i == -1) {
          sprintf(archive_name, "%s%sarc.%s", nsa_path ? nsa_path : "",
                  k == 0 ? "patch/" : "", nsa_archive_ext);
          ai = k == 0 ? (&archive_info_patch) : (&archive_info);
        } else {
          sprintf(archive_name, "%s%sarc%d.%s", nsa_path ? nsa_path : "",
                  k == 0 ? "patch/" : "", i + 1, nsa_archive_ext);
...
size_t NsaReader::getFileLengthSub(ArchiveInfo *ai, const char *file_name) {
  ...
  if (type == NBZ_COMPRESSION || type == SPB_COMPRESSION) {
    ai->fi_list[i].original_length =
        getDecompressedFileLength(type, ai->file_handle, ai->fi_list[i].offset);
  }
...
size_t NsaReader::getFileLength(const char *file_name) {
  ...
  ArchiveInfo *ai[2] = {&archive_info_patch, &archive_info};
  for (int k = 0; k < 2; k++) {
    if ((ret = getFileLengthSub(ai[k], file_name)))
      return ret;

    for (int i = 0; i < num_of_nsa_archives[k]; i++)
      if ((ret = getFileLengthSub(&archive_info2[k][i], file_name)))
        return ret;
  }
...
NsaReader::FileInfo NsaReader::getFileByIndex(unsigned int index) {
  ...
  ArchiveInfo *ai[2] = {&archive_info_patch, &archive_info};
  for (int k = 0; k < 2; k++) {
    if (index < ai[k]->num_of_files)
      return archive_info.fi_list[index];
    index -= ai[k]->num_of_files;

    for (int i = 0; i < num_of_nsa_archives[k]; i++) {
      if (index < archive_info2[k][i].num_of_files)
        return archive_info2[k][i].fi_list[index];
