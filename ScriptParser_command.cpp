...
int ScriptParser::ifCommand() {
  ...
  while (1) {
    if (script_h.compareString("fchk")) {
      script_h.readLabel();
      buf = script_h.readStr();
      f = (script_h.findAndAddLog(script_h.log_info[ScriptHandler::FILE_LOG],
                                  buf, false) != NULL);
      // printf("fchk %s(%d) ", tmp_string_buffer, (findAndAddFileLog(
      // tmp_string_buffer, fasle )) );
    } else if (script_h.compareString("lchk")) {
      script_h.readLabel();
      buf = script_h.readStr();
      f = (script_h.findAndAddLog(script_h.log_info[ScriptHandler::LABEL_LOG],
                                  buf + 1, false) != NULL);
      // printf("lchk %s (%d)\n", buf, f );
    } else {
      int no = script_h.readInt();
...
void ScriptParser::gosubReal(const char *label, char *next_script,
                             bool textgosub_flag, bool pretextgosub_flag) {
