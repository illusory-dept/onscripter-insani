...
void ONScripter::openAudio(int freq) {
  Mix_CloseAudio();
#if (defined(PDA_WIDTH) || defined(PDA_AUTOSIZE)) && !defined(PSP) &&          \
    !defined(IPHONE) && !defined(IOS) && !defined(PANDORA)
  if (Mix_OpenAudio((freq < 0) ? 22050 : freq, MIX_DEFAULT_FORMAT,
                    MIX_DEFAULT_CHANNELS, DEFAULT_AUDIOBUF) < 0) {
#else
  if (Mix_OpenAudio((freq < 0) ? 44100 : freq, MIX_DEFAULT_FORMAT,
                    MIX_DEFAULT_CHANNELS, DEFAULT_AUDIOBUF) < 0) {
#endif
    fprintf(stderr,
...
#else
  fprintf(stderr,
          "--render-font-outline is not supported with SDL_ttf %d.%d.%d\n",
...
char *ONScripter::create_filepath(const char *path_prefix,
                                  const char *filename) {
...
int ONScripter::init() {
  ...
  accumulation_surface =
      AnimationInfo::allocSurface(screen_width, screen_height, texture_format);
  backup_surface =
      AnimationInfo::allocSurface(screen_width, screen_height, texture_format);
  effect_src_surface =
      AnimationInfo::allocSurface(screen_width, screen_height, texture_format);
  ...
  if (file_exists("default.ttf"))
    font_picker = FONT_DEFAULT_TTF;
  else if (file_exists("default.ttc"))
    font_picker = FONT_DEFAULT_TTC;
  else if (file_exists("default.otf"))
    font_picker = FONT_DEFAULT_OTF;
  else if (file_exists("default.otc"))
    font_picker = FONT_DEFAULT_OTC;
  else if (file_exists(archive_default_font_ttf))
    font_picker = FONT_ARCHIVE_TTF;
  else if (file_exists(archive_default_font_ttc))
    font_picker = FONT_ARCHIVE_TTC;
  else if (file_exists(archive_default_font_otf))
    font_picker = FONT_ARCHIVE_OTF;
  else if (file_exists(archive_default_font_otc))
    font_picker = FONT_ARCHIVE_OTC;
#if defined(WIN32)
  else if (file_exists("C:\\Windows\\Fonts\\msgothic.ttc"))
    font_picker = FONT_WIN32_MSGOTHIC_TTC;
  else if (file_exists("C:\\Windows\\Fonts\\msgothic.ttf"))
    font_picker = FONT_WIN32_MSGOTHIC_TTF;
#endif
#if defined(MACOSX)
  else if ([fm fileExistsAtPath:hiraginoPath])
    font_picker = FONT_MACOS_HIRAGINO;
#endif
  ...
  case FONT_DEFAULT_TTF:
    ...
    if (file_exists("default-i.ttf"))
      font_italics_file = create_filepath("", "default-i.ttf");
    else
      faux_italics = true;
    if (file_exists("default-b.ttf"))
      font_bold_file = create_filepath("", "default-b.ttf");
    else
      faux_bold = true;
    if (file_exists("default-bi.ttf"))
      font_bolditalics_file = create_filepath("", "default-bi.ttf");
    else
  ...
  case FONT_DEFAULT_TTC:
    ...
    font_bold_file = font_file;
    font_bolditalics_file = font_file;
    if (file_exists("default-i.ttc"))
      font_italics_file = create_filepath("", "default-i.ttc");
    else
      faux_italics = true;
    if (file_exists("default-b.ttc"))
      font_bold_file = create_filepath("", "default-b.ttc");
    else
      faux_bold = true;
    if (file_exists("default-bi.ttc"))
      font_bolditalics_file = create_filepath("", "default-bi.ttc");
    else
      faux_bolditalics = true;
    break;
  case FONT_DEFAULT_OTF:
    ...
    font_bold_file = font_file;
    font_bolditalics_file = font_file;
    if (file_exists("default-i.otf"))
      font_italics_file = create_filepath("", "default-i.otf");
    else
      faux_italics = true;
    if (file_exists("default-b.otf"))
      font_bold_file = create_filepath("", "default-b.otf");
    else
      faux_bold = true;
    if (file_exists("default-bi.otf"))
      font_bolditalics_file = create_filepath("", "default-bi.otf");
    else
      faux_bolditalics = true;
    break;
  case FONT_DEFAULT_OTC:
    ...
    font_bold_file = font_file;
    font_bolditalics_file = font_file;
    if (file_exists("default-i.otc"))
      font_italics_file = create_filepath("", "default-i.otc");
    else
      faux_italics = true;
    if (file_exists("default-b.otc"))
      font_bold_file = create_filepath("", "default-b.otc");
    else
      faux_bold = true;
    if (file_exists("default-bi.otc"))
      font_bolditalics_file = create_filepath("", "default-bi.otc");
    else
      faux_bolditalics = true;
    break;
  ...
  case FONT_WIN32_MSGOTHIC_TTC:
    ...
    fprintf(stderr,
            "no font file detected; using system fallback (MS Gothic)\n");
    break;
  ...
  case FONT_MACOS_HIRAGINO:
    ...
    faux_italics = faux_bold = faux_bolditalics = true;
    fprintf(stderr,
            "no font file detected; using system fallback (Hiragino Gothic)\n");
    break;
#endif
  default:
    font_picker = -1;
  ...
  if (font_picker == FONT_ARCHIVE_TTF || font_picker == FONT_ARCHIVE_TTC ||
      font_picker == FONT_ARCHIVE_OTF || font_picker == FONT_ARCHIVE_OTC) {
#ifdef USE_FONTCONFIG
    FILE *fp = NULL;
...
#if defined(INSANI)
  if (sentence_font.openFont(font_file, font_bold_file, font_italics_file,
                             font_bolditalics_file, faux_bold, faux_italics,
                             faux_bolditalics, screen_ratio1,
                             screen_ratio2) == NULL) {
#else
...
void ONScripter::flushDirect(SDL_Rect &rect, int refresh_mode) {
  // printf("flush %d: %d %d %d %d\n", refresh_mode, rect.x, rect.y, rect.w,
  // rect.h );
    ...
    if (bl) {
      if (system_menu_mode != SYSTEM_NULL) {
        ...
        if (selectvoice_file_name[SELECTVOICE_OVER])
          playSound(selectvoice_file_name[SELECTVOICE_OVER], SOUND_CHUNK, false,
                    MIX_WAVE_CHANNEL);
      }
  ...
  while (current_line < current_label_info.num_of_lines) {
    if (debug_level > 0)
      printf("*****  executeLabel %s:%d/%d:%d:%d *****\n",
             ...
             current_label_info.num_of_lines, string_buffer_offset,
             display_mode);
...
int ONScripter::parseLine() {
  if (debug_level > 0)
    printf("ONScripter::Parseline %s\n", script_h.getStringBuffer());

#if defined(INSANI)
  // this simply happens to be a function called on a per-line basis -- making
  // it the perfect place to implement a bgmdownmode check
  ...
  if (cmd[0] == ';')
    return RET_CONTINUE;
  else if (strncmp(cmd, "langjp", 6) == 0 && script_h.current_language == 0)
    return RET_CONTINUE;
  else if (strncmp(cmd, "langen", 6) == 0 && script_h.current_language == 1)
    return RET_CONTINUE;
  else if (cmd[0] == '*')
    return RET_CONTINUE;
  else if (cmd[0] == ':')
...
ButtonLink *ONScripter::getSelectableSentence(char *buffer, FontInfo *info,
                                              bool flush_flag,
                                              bool nofile_flag) {
