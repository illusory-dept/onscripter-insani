...
int ONScripter::playSound(const char *filename, int format, bool loop_flag,
  ...
  if (format & SOUND_MUSIC) {
    ...
    if (Mix_PlayMusic(music_info,
                      (music_play_loop_flag && music_loopback_offset == 0.0)
                          ? -1
                          : 0) == 0) {
      music_buffer = buffer;
...
int ONScripter::playMPEG(const char *filename, bool click_flag, bool loop_flag,
  ...
  if (audio_open_flag && !nosound_flag) {
    ...
    char mode[10];
    sscanf(info.audio_string, "MPEG-%d Layer %d %dkbit/s %dHz %s", &mpegversion,
           &layer, &bitrate, &frequency, mode);
    printf("MPEG-%d Layer %d %dkbit/s %dHz %s\n", mpegversion, layer, bitrate,
           frequency, mode);
...
void ONScripter::playClickVoice() {
  if (clickstr_state == CLICK_NEWPAGE) {
    if (clickvoice_file_name[CLICKVOICE_NEWPAGE])
      playSound(clickvoice_file_name[CLICKVOICE_NEWPAGE], SOUND_CHUNK, false,
                MIX_WAVE_CHANNEL);
  } else if (clickstr_state == CLICK_WAIT) {
    if (clickvoice_file_name[CLICKVOICE_NORMAL])
      playSound(clickvoice_file_name[CLICKVOICE_NORMAL], SOUND_CHUNK, false,
                MIX_WAVE_CHANNEL);
  }
}
