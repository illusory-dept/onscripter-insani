...
SDL_Surface *ONScripter::loadImage(char *filename, bool is_flipped,
                                   bool *has_alpha, int *location,
                                   unsigned char *alpha) {
  if (!filename)
    return NULL;
  ...
  if (is_flipped) {
    ...
    SDL_Surface *tmp2 =
        SDL_CreateRGBSurface(SDL_SWSURFACE, tmp->w, tmp->h, fmt->BitsPerPixel,
...
SDL_Surface *ONScripter::createSurfaceFromFile(char *filename, bool *has_alpha,
  ...
  if (filelog_flag)
    script_h.findAndAddLog(script_h.log_info[ScriptHandler::FILE_LOG], filename,
                           true);
...
void ONScripter::alphaBlend(SDL_Surface *mask_surface, int trans_mode,
  ...
  ONSBuf *src1_buffer = (ONSBuf *)effect_src_surface->pixels +
                        effect_src_surface->w * rect.y + rect.x;
  ONSBuf *src2_buffer = (ONSBuf *)effect_dst_surface->pixels +
                        effect_dst_surface->w * rect.y + rect.x;
  ONSBuf *dst_buffer = (ONSBuf *)accumulation_surface->pixels +
                       accumulation_surface->w * rect.y + rect.x;
  ...
  if ((trans_mode == ALPHA_BLEND_FADE_MASK ||
      ...
      mask_surface) {
    for (i = 0; i < rect.h; i++) {
      ONSBuf *mask_buffer = (ONSBuf *)mask_surface->pixels +
  ...
  if (fmt->BitsPerPixel == 16) {
    ...
    if (!rotate_flag) {
      unsigned char *src_buffer =
          (unsigned char *)src_surface->pixels + src_surface->pitch * y2 + x2;
  ...
  for (int i = clip.y; i < clip.y + clip.h; i++) {
    for (int j = clip.x; j < clip.x + clip.w; j++) {
      c = ((((*buf & fmt->Rmask) >> fmt->Rshift) << fmt->Rloss) * 77 +
          ...
          8;
      *buf++ =
          ((monocro_color_lut[c][0] >> fmt->Rloss) << surface->format->Rshift |
...
void ONScripter::refreshSprite(int sprite_no, bool active_flag, int cell_no,
  ...
  if (sprite_info[sprite_no].image_surface &&
      (sprite_info[sprite_no].visible != active_flag ||
       ...
       AnimationInfo::doClipping(check_src_rect, &sprite_info[sprite_no].pos) ==
           0 ||
       AnimationInfo::doClipping(check_dst_rect, &sprite_info[sprite_no].pos) ==
           0)) {
    if (cell_no >= 0)
